...
export function Layout({ children, user, onLogout }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans text-gray-900">
      {/* Mobile Header */}
      <div className="md:hidden bg-black text-white p-4 flex justify-between items-center z-20 shadow-md">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-purple-500 rounded-lg flex items-center justify-center font-bold text-white">
            PEA
          </div>
          <span className="font-semibold text-lg tracking-tight">BPN Inventory</span>
        </div>
        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 hover:bg-gray-800 rounded-md transition-colors">
      ...
      {/* Sidebar */}
      <div className={cn(
        "fixed inset-y-0 left-0 w-64 bg-black text-white transform transition-transform duration-300 ease-in-out z-30 flex flex-col shadow-xl",
        isMobileMenuOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0",
        "md:relative md:flex-shrink-0"
      )}>
        <div className="p-6 hidden md:flex items-center gap-3 border-b border-gray-800">
          <div className="w-10 h-10 bg-purple-500 rounded-xl flex items-center justify-center font-bold text-white shadow-lg shadow-purple-500/20">
            PEA
          </div>
          <div>
            <h1 className="font-bold text-xl tracking-tight">BPN Inventory</h1>
            <p className="text-xs text-purple-300">ระบบจัดการคลังพัสดุ</p>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-gray-800">
          <div className="flex items-center gap-3 px-4 py-3 mb-2 rounded-xl bg-gray-900/50 border border-gray-800">
            <div className="w-8 h-8 rounded-full bg-purple-900/50 flex items-center justify-center text-purple-400 font-medium border border-purple-500/20">
